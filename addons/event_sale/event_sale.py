...
class sale_order_event_attendee(osv.Model):
    _name = 'sale.order.event.attendee'
    _columns = {
...
class sale_order_line(osv.osv):
    ...
    _columns = {
        ...
        'event_attendee_ids': fields.one2many('sale.order.event.attendee', 'sale_order_line_id')
    }
    ...
    def _create_event_registration(self, cr, uid, order_line, values, context=None):
        ...
        registration_id = registration_obj.create(cr, uid, values, context=context)
    ...
    def button_confirm(self, cr, uid, ids, context=None):
        ...
        for order_line in self.browse(cr, uid, ids, context=context):
            if order_line.event_id and not order_line.event_attendee_ids:
                dic = {
                ...
                self._create_event_registration(cr, uid, order_line, dic, context=context)
            elif order_line.event_id and order_line.event_attendee_ids:
                for attendee in order_line.event_attendee_ids:
                    dic = {
                        'name': attendee.name or order_line.order_id.partner_invoice_id.name,
                        'partner_id': order_line.order_id.partner_id.id,
                        'nb_register': 1,
                        'email': attendee.email or order_line.order_id.partner_id.email,
                        'phone': attendee.phone or order_line.order_id.partner_id.phone,
                        'origin': order_line.order_id.name,
                        'event_id': order_line.event_id.id,
                        'event_ticket_id': order_line.event_ticket_id and order_line.event_ticket_id.id or None,
                    }
