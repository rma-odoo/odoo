...
class sale_order_line(osv.osv):
    ...
    def product_id_change(self, cr, uid, ids, pricelist, product, qty=0,
        ...
        if partner_id:
            ...
            lang = partner.lang
            context = dict(context=context, partner_id = partner_id)
            context_partner = dict(context_partner, lang = lang, partner_id = partner_id)
        ...
        result = {}
        product_record = product_obj.browse(cr, uid, product, context=context_partner)

        uom2 = False
        if uom:
            uom2 = product_uom_obj.browse(cr, uid, uom)
            if product_record.uom_id.category_id.id != uom2.category_id.id:
                uom = False
        if uos:
            if product_record.uos_id:
                uos2 = product_uom_obj.browse(cr, uid, uos)
                if product_record.uos_id.category_id.id != uos2.category_id.id:
                    uos = False
            else:
                uos = False
        ...
        if update_tax: #The quantity only have changed
            result['tax_id'] = self.pool.get('account.fiscal.position').map_tax(cr, uid, fpos, product_record.taxes_id)

        if not flag:
            result['name'] = self.pool.get('product.product').name_get(cr, uid, [product_record.id], context=context_partner)[0][1]
            if product_record.description_sale:
                result['name'] += '\n'+product_record.description_sale
        domain = {}
        if (not uom) and (not uos):
            result['product_uom'] = product_record.uom_id.id
            if product_record.uos_id:
                result['product_uos'] = product_record.uos_id.id
                result['product_uos_qty'] = qty * product_record.uos_coeff
                uos_category_id = product_record.uos_id.category_id.id
            else:
                result['product_uos'] = False
                result['product_uos_qty'] = qty
                uos_category_id = False
            result['th_weight'] = qty * product_record.weight
            domain = {'product_uom':
                        [('category_id', '=', product_record.uom_id.category_id.id)],
                        'product_uos':
                        [('category_id', '=', uos_category_id)]}
        elif uos and not uom: # only happens if uom is False
            result['product_uom'] = product_record.uom_id and product_record.uom_id.id
            result['product_uom_qty'] = qty_uos / product_record.uos_coeff
            result['th_weight'] = result['product_uom_qty'] * product_record.weight
        elif uom: # whether uos is set or not
            default_uom = product_record.uom_id and product_record.uom_id.id
            ...
            if product_record.uos_id:
                ...
                result['product_uos'] = False
                result['product_uos_qty'] = qty
            result['th_weight'] = q * product_record.weight        # Round the quantity up

        if not uom2:
            uom2 = product_record.uom_id
        ...
        if (pricelist and partner_id):
            price = self.pool.get('product.pricelist').price_get(cr, uid, [pricelist],
             ...
             price = product_obj.price_get(cr, uid, [product], ptype='list_price', context=context)[product] or False
        result.update({'price_unit': price})
        return {'value': result, 'domain': domain, 'warning': False}
